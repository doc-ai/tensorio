...
class TF20KerasMNISTFashionTest {
    ...
    fun testSavedModel() {
        try {
            val bundle = bundleForFile("keras-mnist-fashion-save-model.tiobundle", "tf_2_0")
    ...
    fun testModelBuilder() {
        try {
            val bundle = bundleForFile("keras-mnist-fashion-model-builder.tiobundle", "tf_2_0")
            Assert.assertNotNull(bundle)
    ...
    fun testEstimatorPredict() {
        try {
            val bundle = bundleForFile("keras-mnist-fashion-estimator-predict.tiobundle", "tf_2_0")
            Assert.assertNotNull(bundle)
    ...
    fun testEstimatorTrain() {
        try {
            val bundle = bundleForFile("keras-mnist-fashion-estimator-train.tiobundle", "tf_2_0")
            Assert.assertNotNull(bundle)
            ...
            for (epoch in 0 until 4) {
                ...
                Assert.assertNotNull(output)
                val loss = output.get("sparse_categorical_crossentropy/weighted_loss/value")
